...
- **노션(Notion) 스타일 웹 대시보드**: 깔끔하고 직관적인 UI와 다크/라이트 모드 테마를 지원합니다. 복잡한 설정 파일 수정 없이 클릭만으로 모델을 관리하세요.
- **클라우드를 위한 영구 세션 관리**: GCP e2-micro 같은 저사양 서버에서도 돌아가도록 설계되었습니다. 유휴 브라우저 탭을 강제 종료하여 RAM을 아끼고, 대시보드를 통해 손쉽게 세션 파일을 업로드하여 GUI 없이도 영구적인 로그인을 유지합니다.
- **실시간 모델 동기화**: 백그라운드 스크래퍼가 각 웹사이트에서 제공하는 최신 모델 목록을 자동으로 파싱하여 동기화합니다.
...
│   ├── api/
│   │   ├── routes/
│   │   │   ├── admin.py          # 관리자용 대시보드 API (수정, 상태 변경, 업로드)
│   │   │   ├── chat.py           # OpenAI 형식 /v1/chat/completions 대응
│   │   │   └── models.py
│   │   ├── static/               # Admin 대시보드 웹앱 (노션 스타일)
│   │   └── schemas.py
│   ├── core/
│   │   ├── browser_pool.py       # Playwright 브라우저 생명주기 및 메모리 관리
...
flowchart TD
    ...
    L --> M([5분간 유휴 시 RAM 확보를 위해 Context 자동 종료])
```
...
1. **설정 활성화**: 좌측 내비게이션 바에서 두 번째 'Settings' 탭을 누릅니다. 사용하고자 하는 모듈의 우측 토글 스위치를 활성화시킵니다.
2. **로컬 세션 연동 (내 PC에서 돌릴 때)**:
...
### 3. 클라우드(GCP 등)에 배포하기 위한 영구 세션 설정 가이드

GCP 같은 클라우드 서버는 모니터 화면이 없어서 위와 같은 직접 로그인이 불가능합니다. 24시간 끊김없이 사용하려면 다음 절차를 따르세요:
1. 먼저 내 PC(로컬)에서 proXiPT를 켜고, 위 설명대로 `Login (GUI)` 버튼을 눌러 로그인을 완료합니다.
2. 내 PC의 폴더 안에 `sessions/` 폴더가 생기고, 그 안에 `chatgpt_session.json` 같은 파일이 만들어집니다.
3. 이제 클라우드 서버에 proXiPT를 설치하고 실행시킵니다.
4. 클라우드 서버의 대시보드에 접속한 뒤, **"Upload JSON"** 버튼을 누르고 내 PC에서 만들어진 세션 파일을 업로드합니다.
5. 끝입니다! 서버가 주기적으로 쿠키를 파일에 덮어쓰며 영구적으로 세션을 유지합니다.
