...
class PoeProvider(BaseProvider):
    ...
    SEL_TEXTAREA = 'textarea[class*="TextArea"], textarea, div[contenteditable="true"]'
    SEL_SEND = 'button[class*="Send"], button[aria-label*="Send"]'
    SEL_RESPONSE = 'div[class*="Message_botMessage"] .markdown, div[class*="bot_message"], div[class*="response"]'
    SEL_NEW_CHAT = 'button[class*="Clear"], a[class*="new"]'

    async def ensure_ready(self, page: Page) -> None:
        url = page.url
        if not url or "poe.com" not in url:
            log.info("Navigating to Poe")
        ...
        try:
            ...
            log.warning("Poe input not found — login may be needed")
    ...
    async def detect_error(self, page: Page) -> str | None:
        ...
        lower = content.lower()
        if any(p in lower for p in ["rate limit", "daily limit", "out of credits"]):
            return "rate_limit"
        if any(p in lower for p in ["log in", "sign up", "login"]):
            try:
