"""Doubao provider — doubao.com (ByteDance)"""
...
log = get_logger("provider.doubao")
...
class DoubaoProvider(BaseProvider):
    ...
    SEL_TEXTAREA = 'textarea, div[contenteditable="true"], div.editor'
    ...
    SEL_NEW_CHAT = 'button[class*="new"], a[href*="/chat"], div[class*="new-chat"]'

    async def ensure_ready(self, page: Page) -> None:
        url = page.url
        if not url or "doubao.com" not in url:
            log.info("Navigating to Doubao")
        ...
        try:
            ...
            log.warning("Doubao input not found")
    ...
    async def detect_error(self, page: Page) -> str | None:
        ...
        lower = content.lower()
        if any(p in lower for p in ["请求过于频繁", "rate limit", "too many", "稍后再试", "服务繁忙"]):
            return "rate_limit"
        if any(p in lower for p in ["验证", "captcha"]):
            return "captcha"
        if any(p in lower for p in ["请登录", "login", "sign in"]):
            try:
