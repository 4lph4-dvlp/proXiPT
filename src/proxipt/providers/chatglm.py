"""ChatGLM / Zhipu provider — chatglm.cn"""
...
log = get_logger("provider.chatglm")
...
class ChatGLMProvider(BaseProvider):
    ...
    SEL_TEXTAREA = 'textarea, div[contenteditable="true"], div.input-box'
    SEL_SEND = 'button[class*="send"], button[aria-label*="Send"], button[type="submit"]'
    SEL_RESPONSE = 'div[class*="message"][class*="bot"] .markdown, div[class*="answer"], div[class*="response"]'
    SEL_NEW_CHAT = 'button[class*="new"], a[href*="/main"], div[class*="new"]'

    async def ensure_ready(self, page: Page) -> None:
        url = page.url
        if not url or "chatglm.cn" not in url:
            log.info("Navigating to ChatGLM")
        ...
        try:
            ...
            log.warning("ChatGLM input not found")
    ...
    async def detect_error(self, page: Page) -> str | None:
        ...
        if any(p in lower for p in ["验证", "captcha"]):
            return "captcha"
        if any(p in lower for p in ["请登录", "login"]):
            try:
