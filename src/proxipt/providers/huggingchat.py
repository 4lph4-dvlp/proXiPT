"""HuggingChat provider — huggingface.co/chat"""
...
log = get_logger("provider.huggingchat")
...
class HuggingChatProvider(BaseProvider):
    ...
    SEL_TEXTAREA = 'textarea[placeholder*="Ask"], textarea'
    SEL_SEND = 'button[type="submit"], button[aria-label*="Send"]'
    SEL_RESPONSE = 'div.prose, div[class*="message"][class*="assistant"], div.markdown'
    SEL_NEW_CHAT = 'a[href="/chat"], button[title*="New Chat"]'

    async def ensure_ready(self, page: Page) -> None:
        url = page.url
        if not url or "huggingface.co/chat" not in url:
            log.info("Navigating to HuggingChat")
        ...
        try:
            ...
            log.warning("HuggingChat input not found")
